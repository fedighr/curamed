...
$query = "SELECT h.*, 
          ...
          FROM historique h
          LEFT JOIN utilisateur m ON h.id_medecin = m.id_utilisateur
          LEFT JOIN utilisateur p ON h.id_patient = p.id_utilisateur
          WHERE h." . ($user_type === 'medecin' ? 'id_medecin' : 'id_patient') . " = ? 
...
<section><h3>.</h3></section>
    <main class="dashboard-main5">
        <div class="container5">
            <div class="section-header5">
                <h2 class="section-title5">Historique des Consultations : </h2>
                
            </div>

            <?php if(empty($historiques)): ?>
                <div class="empty-state5">
                    <i class="fas fa-file-medical empty-history-icon5"></i>
                    <p>Aucune consultation trouvée dans votre historique</p>
                </div>
            <?php else: ?>
                <div class="table-responsive5">
                    <table class="calendar-table5">
                        ...
                        <tbody>
                            <?php foreach ($historiques as $consultation): ?>
                            <tr>
                                <td data-label="Date">
                                    <?= htmlspecialchars(date('d/m/Y H:i', strtotime($consultation['date_consultation']))) ?>
                                </td>
                                <td data-label="Médecin">
                                    <?= htmlspecialchars($consultation['medecin_prenom'] . ' ' . $consultation['medecin_nom']) ?>
                                </td>
                                <td data-label="Patient">
                                    <?= htmlspecialchars($consultation['patient_prenom'] . ' ' . $consultation['patient_nom']) ?>
                                </td>
